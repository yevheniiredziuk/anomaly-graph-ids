...
/**
 * Section 6.4 — Neo4j "Louvain community detection". Full cycle: project → Louvain → drop.
 *
 * <p>GDS 2026.03 rejects the {@code randomSeed} config key for Louvain (same finding as T10-T13);
 * determinism is provided by {@code concurrency: 1}.
 */
...
public class Neo4jLouvainBenchmark {
  ...
  private static final String LOUVAIN =
      """
      CALL gds.louvain.stream($name, {maxIterations: 10, tolerance: 0.0001, concurrency: 1})
      YIELD nodeId, communityId RETURN count(*) AS n
      """;
  ...
  @Benchmark
  public long louvainFullCycle(InfrastructureState infra, WindowSampler sampler) {
    ...
    try (Session session = infra.neo4jDriver.session()) {
      ...
      if (nodeCount > 0) {
        try {
          result =
              session.executeWrite(
                  tx -> tx.run(LOUVAIN, Map.of("name", name)).single().get("n").asLong());
        } catch (Exception e) {
          result = -1; // degenerate graph — swallow to keep JMH IPC serializable
        }
      }
    } finally {
      try (Session session = infra.neo4jDriver.session()) {
