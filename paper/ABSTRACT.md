...
**Результати.** Метод перевірено на підмножині публічного датасету CICIDS2017 (понеділок-середа, з очищенням за Engelen та ін.). У прийнятому host-window протоколі оцінювання ($\Delta t = 5$ хв) графовий метод досягає загальних метрик $P = 0{,}006$, $R = 0{,}098$, $F_1 = 0{,}011$ проти $F_1 = 0{,}080$ для flow-centric baseline і не перевершує baseline у чутливості виявлення в жодному з протестованих класів атак; водночас метод надає інтерпретовані структурні сигнали, недоступні flow-centric підходу, та конкурентну p95-латентність на спеціалізованих графових операціях (approximate betweenness 74,6 мс, Louvain community detection 219,0 мс за 5-хв вікно). Ablation-аналіз виявляє, що найбільш інформативним компонентом композитної метрики є відхилення betweenness centrality ($\alpha_1$), тоді як Jaccard drift 2-hop околиці ($\alpha_3$) не додає корисного сигналу у прийнятому протоколі. Усі компоненти метрики мають прозору топологічну інтерпретацію, що забезпечує пояснюваність рішень для операторів SOC.
...
**Results.** The method is evaluated on a subset of the public CICIDS2017 dataset (Monday-Wednesday, cleaned according to Engelen et al.). Under the adopted host-window evaluation protocol ($\Delta t = 5$ min), the graph-based method attains overall $P = 0.006$, $R = 0.098$, $F_1 = 0.011$ against $F_1 = 0.080$ for the flow-centric baseline, and does not exceed the baseline in detection recall on any of the tested attack classes; it nevertheless delivers interpretable structural signals unavailable to the flow-centric approach and competitive p95 latency on graph-specific operations (approximate betweenness 74.6 ms, Louvain community detection 219.0 ms per 5-min window). An ablation study identifies the betweenness-centrality deviation ($\alpha_1$) as the most informative component of the composite metric, while the 2-hop-neighborhood Jaccard drift ($\alpha_3$) does not add a useful signal under the adopted protocol. All components of the composite metric retain explicit topological interpretation, providing decision explainability for SOC operators.
